...
/** Narxni "raqam so'm (so'zda so'm)" ko'rinishida */
export function priceInWordsUz(value: number): string {
  return `${numberToWordsUz(value)} so'm`;
}
